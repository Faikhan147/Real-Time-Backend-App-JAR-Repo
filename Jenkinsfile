...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SONAR_PROJECT_KEY', defaultValue: 'Jar-App', description: 'SonarQube Project Key')
        string(name: 'SONAR_PROJECT_NAME', defaultValue: 'Backend-Jar-App', description: 'SonarQube Project Name')
...
stage('Create SonarQube Project') {
    steps {
        withCredentials([string(credentialsId: 'Sonar-Global-Token', variable: 'SONAR_TOKEN')]) {
            sh """
                curl -u ${SONAR_TOKEN}: -X POST "http://52.66.244.246:9000/api/projects/create?name=${SONAR_PROJECT_NAME}&project=${SONAR_PROJECT_KEY}"
            """
        }
    }
}
                    ...
                    dir('JAR-Project') {
                        script {
                            ...
                            sh """
                                ...
                                -Dsonar.host.url=http://52.66.244.246:9000 -X
                            """
                        }
                    }
                }
        ...
        script {
            ...
            retry(3) {
                withEnv(["JAR_APP_URL=$JAR_APP_URL"]) {
                    sh '''#!/bin/bash
                    echo "🌐 Performing Jar App health check on $JAR_APP_URL ..."
                    sleep 10  # wait for service to be ready
                    echo "🔍 DNS resolution test:"
                    nslookup "$JAR_APP_URL" || echo "❗ DNS resolution failed."

                    echo "🔍 CURL with verbose output:"
                    curl -v "$JAR_APP_URL" || echo "❗ CURL failed."

                    STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$JAR_APP_URL")
                    echo "ℹ️ HTTP Status Code: $STATUS_CODE"

                    if [ "$STATUS_CODE" -ne 200 ]; then
                        echo "❌ Jar App health check failed with status code $STATUS_CODE"
                        exit 1
                    fi
